...
export class HomePage {
  ...
  places: { title: string }[] = [];
  constructor(private screenshot: Screenshot,private fingerAuth: FingerprintAIO, public toastCtrl: ToastController, public popoverCtrl: PopoverController, public navCtrl: NavController, private placesService: PlacesService, public actionSheetCtrl: ActionSheetController, public alertCtrl: AlertController, public modalCtrl: ModalController) {
  }
    ...
    this.fingerAuth.isAvailable().then(result => {
      if (result === "OK") {
        this.fingerAuth.show(this.fingerprintOptions)
          .then((result: any) => this.showalert('Authen success'))
  ...
  takePic() {
    this.screenshot.save('jpg', 80, Math.random()+'.jpg').then(this.onSuccess,this.onError);
  }
  onChange(carbrand) {
